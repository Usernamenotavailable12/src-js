  ...
  let fortuneWheels = [];
  let spunBoxes = []; // New: Track boxes that have already been spun
  ...
  `;
    ...
    // Filter out any boxes that have already been spun (using our local state)
  ...
  async function startWheelSpin() {
    ...
    try {
      ...
      if (winningRewardIndex === -1) {
        // If we can’t find a matching reward index, handle gracefully
