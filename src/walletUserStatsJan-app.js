  ...
  async function showBetProgressJan() {
    ...
    try {
      ...
      displayProgressBarsJan(totalBet);
    } catch (error) {
      alert("Failed to fetch progress data.");
    }
  }

  function displayProgressBarsJan(totalBet) {
    ...
    const stage1Percentage = Math.min((totalBet / 8000) * 100, 100);
    const stage2Percentage =
      totalBet > 8000 ? Math.min(((totalBet - 8000) / 12000) * 100, 100) : 0;
    const stage3Percentage =
      totalBet > 20000 ? Math.min(((totalBet - 20000) / 30000) * 100, 100) : 0;
