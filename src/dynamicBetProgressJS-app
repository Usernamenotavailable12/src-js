    ...
    function buildProgressBar(progressElem, parent, data) {
      const progress = calculatePercent(progressElem.range.start, progressElem.range.end, data);

      const progressItemsHTML = progressElem.progress.map(element => {
        const cleanText = element.text.toLowerCase().replace(/[^a-z0-9_-]/g, '');
        const className = progress > element.percent ? `open-present-${cleanText} progress-achieved` : `present-${cleanText}`;
      ...
      const progressHTML = `
                ...
                <div class="progressbar progress-${progressElem.range.end}" style="position: relative; width: 100%;">
                    <div class="activeProgress ${activeProgressClass}" style="${activeProgressStyle}"></div>
                    ${progressItemsHTML}
                </div>
            `;
